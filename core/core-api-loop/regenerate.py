...
def slug_md_path(slug: str) -> str:
    return os.path.join(REPO_ROOT, "compliance", "policies", slug, f"{slug}.md")
...
def cmd_measure(args) -> None:
    ...
    if args.slug or args.spec_scored:
        ec, fp = control_oracle.code_sets(control_oracle.parse_vocab(
            os.path.join(REPO_ROOT, "core", "core-api.yaml"),
...
def select_affected(args) -> dict:
    ...
    for slug, rec in sorted(by_slug.items()):
        ...
        rows.append({
            "slug": slug,
            "regeneratable": os.path.exists(os.path.join(REPO_ROOT, "compliance", "policies", slug, "prompt.md")),
