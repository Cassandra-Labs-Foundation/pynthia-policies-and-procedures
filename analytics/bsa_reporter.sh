...
# money movement over the trailing 90 days ACROSS BOTH TIERS (hot Postgres +
# cold Parquet, card 62's spanning view) and writes a SAR candidate row back
# to Postgres when the pattern holds at that horizon.
#
...
lookback as (
  select f.entity_hash, f.instance_id,
  ...
  where e.received_at > now() - interval 90 day
    and e.amount_cents < 1000000
  group by 1, 2
  having sum(e.amount_cents) >= 1000000
)
select l.entity_hash, l.instance_id, l.window_start, current_date,
       l.total_cents, l.event_count,
       '90-day lookback: ' || l.event_count || ' sub-threshold events totalling '
