...
| Suspected inquiry-stage discouragement reported (`fair_lending.discouragement_reported`) | Report detail and channel (`complaint.inquiry_detail`), staff/branch involved (`fair_lending.report_subject`) | Triage record; escalation to CCO with disposition and remediation (`fair_lending.remediation_opened`) | Triage within 5 BD; disposition within 30 days (internal) |
...
| When | What's needed | Produced (and logged) | Within |
|---|---|---|---|
| Application decisioned (`loan_application.decisioned`) | Registered model or judgmental criteria ID (`decision.model_id`), inputs used (`decision.inputs[]`), income classifications (`applicant.income_sources[]`) | Decision record with basis and factor weights (`decision.recorded`) | At decision (—) |
| Pricing deviates from rate sheet (`pricing.exception_requested`) | Standard price (`pricing.sheet_price`), proposed price (`pricing.proposed_price`), rationale (`pricing.exception_rationale`), approver (`pricing.exception_approver`) | Approved/denied exception record; funding blocked until decided (`pricing.exception_decided`) | Before funding (—) |
...
| Accessibility accommodation requested in a marketing or application flow (`accessibility.accommodation_requested`) | Request detail and channel (`accessibility.request_detail`), applicant contact (`applicant.contact`) | Accommodation provided and logged (`accessibility.accommodation_provided`) | Promptly (internal: 3 BD) |
| Annual reach-assessment cycle opens (`advertising.reach_review_due`) | Ad inventory and spend by medium (`advertising.media_inventory[]`), geo-targeting history (`ad.geo_filters[]`), intermediary list (`advertising.intermediaries[]`), market demographics (`analytics.assessment_area`) | Demographic-reach assessment with findings and any CAP (`advertising.reach_review_completed`) | Annually, aligned with the Q1 redlining review (—) |
...
- **Owner:** Patrick Wilson, Chief Compliance Officer — accountable for this policy, the guarded-attribute list, disparity thresholds, exception and waiver authority, discouragement-report dispositions, and CAP closure.
...
- **Engineering vocabulary is provisional for a defined set of codes.** The parsed `vocabulary.json` (Cassandra Banking Core API v2.2.0) now registers most lending-side events used here (`application.*`, `loan_application.*`, `aan.*`, `notice.*`, `valuation.*`, `pricing.*`, `hmda.*`, `ad.*`, `advertising.*`, `lo_comp.*`, `vendor.*`, `analytics.*`, `training.*`, `record.*`, `rule.*`, `form.*`, `facility.*`, `governance.*`, `fair_lending.*`). Codes still missing from vocabulary and used here as the target naming scheme include: `loan_application.oral_adverse_decision` (registered only as a field, not an event), `fair_lending.discouragement_reported`, `fair_lending.report_subject`, `accessibility.accommodation_requested`, `accessibility.accommodation_provided`, `accessibility.request_detail`, `advertising.reach_review_due`, `advertising.reach_review_completed`, `advertising.media_inventory[]`, `advertising.intermediaries[]`, `ad.accessibility_check`, `compliance.guardrails_published`, `rule.change_submitted`, `rule.fields_referenced[]`, `rule.guardrail_check_completed`, `compliance.guarded_attributes[]`, `vendor.cap_issued`, `vendor.mi_trends`, `vendor.cap_history[]`, `vendor.mi_due_at`, `vendor.dd_materials[]`, the timer codes `application.notice_due_at`, `account.aan_due_at`, `application.counteroffer_due_at`, `valuation.rov_due_at`, `pricing.exception_review_due_at`, `hmda.lar_qc_due_at`, `hmda.submission_due_at`, `analytics.disparity_due_at`, `analytics.redlining_due_at`, `training.onboarding_due_at`, `training.annual_due_at`, `record.retention_expires_at`, `facility.signage_review_due_at`, and assorted input fields (`form.template_diff`, `form.question_inventory[]`, `application.fields_collected[]`, `application.disclosure_timestamps[]`, `application.reason_codes[]`, `application.missing_items[]`, `application.eligible_options[]`, `application.option_economics[]`, `applicant.state`, `applicant.business_revenue_tier`, `applicant.income_sources[]`, `decision.model_id`, `decision.inputs[]`, `property.address`, `valuation.panel_roster[]`, `valuation.report`, `valuation.rov_evidence[]`, `pricing.sheet_price`, `pricing.proposed_price`, `pricing.exceptions[]`, `hmda.lar_fields[]`, `hmda.lar_rows[]`, `hmda.source_records[]`, `hmda.reporter_id`, `notice.sent_at`, `record.hold_matter_id`, `record.hold_status`, `training.assignee_id`, `training.change_summary`, `training.affected_roles[]`, `training.completion_status`, `training.assessment_score`, `training.population_roster[]`). Engineering will register or remap these before the next review.
