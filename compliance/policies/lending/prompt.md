...
(`LP-09`). Before generating, read `shared-controls/record-retention-mechanics.md` at
...
Do not renumber this control into the LP-xx sequence, do not retitle it, and do not
...
`LP-09` (the local retention control) covers only: the credit package completeness gate
(`loan_application.doc_block_state` hard-block before booking), clock-setting at final
action (`credit_package.retention.started`), and ALERTS for incomplete packages and
missing expiry dates. Do NOT include legal-hold, destruction, or permanent-record EVENTS
in `LP-09` — those live exclusively in SC-02.
