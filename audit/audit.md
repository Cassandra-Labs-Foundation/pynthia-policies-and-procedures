...
**ALERTS/METRICS:** Alert fires 90 days before `audit.retention_expires_at` to allow legal hold review before destruction. Alert fires if `audit.workpaper_access.requested` has no `audit.workpaper_access.decided` within 5 BD (target: zero unresolved access requests). Dashboard tracks count of work paper sets approaching retention expiry. Hold, destruction, and permanent-record alerts are governed by [SC-02](#sc-02-record-retention-lifecycle-mechanics).
